  ...
  const speakers: SpeakerData[] =[
    {
      ...
      name: "Ricardo Caldas",
      role: "Product Owner",
      company: "VW Group Digital Solutions",
  ...
  const KeynoteCard = ({ data }: { data: SpeakerData }) => {
    ...
    return (
      <div className="group relative overflow-hidden rounded-[2rem] aspect-[3/4] w-full max-w-[320px] mx-auto shadow-xl bg-brand-darkBlue">
        ...
        {/* Selo English (Glassmorphism) */}
        {data.isEnglish && (
            <div className="absolute top-4 right-4 bg-black/40 backdrop-blur-md border border-white/20 text-white text-[10px] font-bold px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-lg z-20">
    ...
    return (
      <div className="flex flex-col items-center text-center group">
        <div className="relative w-full aspect-square rounded-2xl overflow-hidden mb-4 shadow-md border border-gray-100 bg-gray-100">
          ...
          {/* Apenas o LinkedIn no Hover */}
          {data.linkedin && (
            <a 
              href={data.linkedin} 
              target="_blank" 
              rel="noreferrer" 
              className="absolute bottom-3 right-3 w-8 h-8 rounded-full bg-brand-darkBlue text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 hover:bg-brand-blue shadow-lg z-20" 
              aria-label={`LinkedIn de ${data.name}`}
            >
              <Linkedin className="w-4 h-4" />
            </a>
